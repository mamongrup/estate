...
const catalogUi = text => window.MamonI18n?.t(text) || text;
    ...
    <article class="property-card">
      <a class="property-link" href="/ilan/${esc(x.id)}">
        ...
        <div class="property-body">
          ...
          <h3>${esc(x.title)}</h3>
          <div class="features">
            <span>${esc(x.rooms)} ${catalogUi('Oda')}</span>
            <span>${esc(x.bath)} ${catalogUi('Banyo')}</span>
            <span>${esc(x.area)} m²</span>
          </div>
          <div class="property-price">
            <b>${new Intl.NumberFormat(window.MamonI18n?.locale || 'tr-TR', { maximumFractionDigits: 0 }).format(Number(x.price || 0) * (window.MAMON_RATES?.[currentCurrency] || 1))} ${catalogSymbols[currentCurrency] || currentCurrency}</b>
